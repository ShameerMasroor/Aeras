...
void DroneSimpleControllerPrivate::InitSubscribers(
  ...
  std::string cmd_quat_topic_) // Add default parameter for SLAM pose topic
{
  ...
  if (!slam_pose_topic_.empty()) {
    slam_pose_subscriber_ = ros_node_->create_subscription<geometry_msgs::msg::Pose>(
      slam_pose_topic_, qos,
      [this](const geometry_msgs::msg::Pose::SharedPtr msg) {
        slam_pose_ = *msg; // Update the SLAM pose
      });
  } else {
    RCLCPP_ERROR(ros_node_->get_logger(), "No SLAM pose topic defined!");
  }

  // Subscribe to quaternion command topic (cmd_quat)
  if (!cmd_quat_topic_.empty()) {
    cmd_quat_subscriber_ = ros_node_->create_subscription<geometry_msgs::msg::Quaternion>(
        cmd_quat_topic_, qos,
        std::bind(&DroneSimpleControllerPrivate::CmdQuatCallback, this, std::placeholders::_1));
  } else {
      RCLCPP_ERROR(ros_node_->get_logger(), "No cmd_quat topic defined!");
  }
...
void DroneSimpleControllerPrivate::CmdQuatCallback(const geometry_msgs::msg::Quaternion::SharedPtr msg)
{
    // Store the received quaternion
    cmd_quat = *msg;

    // Optionally, log the received quaternion for debugging purposes
    RCLCPP_INFO(ros_node_->get_logger(), "Received Quaternion: [w: %f, x: %f, y: %f, z: %f]",
                cmd_quat.w, cmd_quat.x, cmd_quat.y, cmd_quat.z);
}
...
void DroneSimpleControllerPrivate::UpdateDynamics(double dt)
{
  ...
  // Publish the SLAM pose of the drone to the ROS topic
    ...
    pub_gt_pose_->publish(gt_pose); // Fallback to publishing the ground truth pose
  



  // Replace ground truth pose with SLAM pose

  if (slam_pose_subscriber_) { // Ensure the SLAM pose subscriber is active
  
    pose = ignition::math::v6::Pose3d(
    slam_pose_.position.x, slam_pose_.position.y, slam_pose_.position.z,
    slam_pose_.orientation.w, slam_pose_.orientation.x, slam_pose_.orientation.y, slam_pose_.orientation.z);
    // RCLCPP_INFO(ros_node_->get_logger(), "SLAM pose used.");

  } else {
    RCLCPP_WARN(ros_node_->get_logger(), "SLAM pose not available. Ground truth pose used.");
  }
...
if (m_posCtrl) {
  ...
  if (navi_state == FLYING_MODEL) {
    ...
    //Euler[2] is the euler angle about the z axis
    //torque[2] is the torque applied about the z axis

    /* This Yaw controller was added by Zarmasha, which is a abrevatiation of Zaryan, Maleeha, and Shameer
    This controller is the property of the said FYP group and may be used for academic purposes only.
    January, 17, 2025 */
    double desired_yaw = atan2(2.0 * (cmd_quat.w * cmd_quat.z), 1.0 - 2.0 * (cmd_quat.z * cmd_quat.z)); //extract the yaw
    double yaw_error = desired_yaw - euler[2];
    ...
    double yaw_diff = (yaw_error - last_yaw) / dt; //differential
    double control_signal = inertia[2] * (Kp * yaw_error + Kd * yaw_diff);

    torque[2] = control_signal; // writing control signal to the torque 
    last_yaw = yaw_error; //updating last stored yaw
