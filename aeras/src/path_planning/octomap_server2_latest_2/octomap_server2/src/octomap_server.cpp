...
namespace octomap_server {
    OctomapServer::OctomapServer(
        ...
        m_baseFrameId("simple_drone/base_footprint"),
        m_useHeightMap(true),
        ...
        m_groundFilterPlaneDistance(1.00),
        m_compressMap(true),
        m_incrementalUpdate(true) {

        // rclcpp::Clock::SharedPtr clock = std::make_shared<rclcpp::Clock>(RCL_SYSTEM_TIME);
        rclcpp::Clock::SharedPtr clock = this->get_clock();
        this->buffer_ = std::make_shared<tf2_ros::Buffer>(clock,std::chrono::seconds(10));
        ...
        double probHit = this->declare_parameter("sensor_model/hit", 0.8);
        double probMiss = this->declare_parameter("sensor_model/miss", 0.3);
    ...
    void OctomapServer::insertScan(
        ...
        for (auto it = occupied_cells.begin(), end = occupied_cells.end(); it != end; it++) {
            ...
            octomap::point3d occupiedPoint = m_octree->keyToCoord(*it);
        
            // Inflate by adding occupied neighbors
            for (double dx = -inflation_radius; dx <= inflation_radius; dx += m_octree->getResolution()) {
                for (double dy = -inflation_radius; dy <= inflation_radius; dy += m_octree->getResolution()) {
                    for (double dz = -inflation_radius; dz <= inflation_radius; dz += m_octree->getResolution()) {
                        if (dx == 0 && dy == 0 && dz == 0) continue; // Skip original voxel
        
                        octomap::point3d inflatedPoint(occupiedPoint.x() + dx, 
                                                       occupiedPoint.y() + dy, 
                                                       occupiedPoint.z() + dz);
                        octomap::OcTreeKey inflatedKey;
                        if (m_octree->coordToKeyChecked(inflatedPoint, inflatedKey)) {
                            m_octree->updateNode(inflatedKey, true); // Mark as occupied
                        }
                    }
                }
            }
